...
# ---------------------------------------------------------
# RELIABLE CONTAINER PROVISIONING ENGINE (NO LXCFS NEEDED)
# ---------------------------------------------------------
def _provision_vps_sync(image_tag: str, ram_bytes: int, cpu_cores: int, disk_bytes: int, container_name: str, root_password: str):
    ...
    # Command script to emulate hardware specs inside termux/terminus/neofetch without lxcfs
    startup_script = (
        "apt-get update && apt-get install -y openssh-server neofetch curl procps && "
        "mkdir -p /var/run/sshd && "
        f"echo \"root:{root_password}\" | chpasswd && "
        "sed -i 's/#PermitRootLogin.*/PermitRootLogin yes/g' /etc/ssh/sshd_config && "
        f"echo \"MemTotal:       {ram_kb} kB\" > /fake_mem && "
        f"echo \"MemFree:        {ram_kb - 102400} kB\" >> /fake_mem && "
        f"echo \"MemAvailable:   {ram_kb - 51200} kB\" >> /fake_mem && "
        f"echo \"Buffers:        10240 kB\" >> /fake_mem && "
        f"echo \"Cached:         30720 kB\" >> /fake_mem && "
        "mount --bind /fake_mem /proc/meminfo 2>/dev/null || true && "
        f"echo -e '#!/bin/bash\\necho {cpu_cores}' > /usr/bin/nproc && chmod +x /usr/bin/nproc && "
        f"echo -e '#!/bin/bash\\necho \"               total        used        free      shared  buff/cache   available\"\\necho \"Mem:           {ram_mb}         100        {ram_mb-100}           0          40       {ram_mb-50}\"\\necho \"Swap:             0           0           0\"' > /usr/bin/free && chmod +x /usr/bin/free && "
        "/usr/sbin/sshd -D"
    )
    ...
    container = docker_client.containers.run(
        image=image_tag,
        name=container_name,
        command=f"bash -c \"{startup_script}\"",
        detach=True,
        tty=True,
        stdin_open=True,
        mem_limit=ram_bytes,
