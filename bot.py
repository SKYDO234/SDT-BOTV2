...
# ---------------------------------------------------------
# TAILSCALE & FAST CONTAINER ENGINE
# ---------------------------------------------------------
def _provision_vps_sync(image_tag: str, ram_bytes: int, cpu_cores: int, container_name: str, root_password: str):
    ...
    ts_container = docker_client.containers.run(
        ...
        volumes={
            state_dir: {"bind": "/var/lib/tailscale", "mode": "rw"}
        },
        restart_policy={"Name": "unless-stopped"}
    )

    # 3. Extract Login URL directly
    login_url = None
    for _ in range(10):
    ...
    if not login_url:
        logs = ts_container.logs(tail=50).decode("utf-8", errors="ignore")
        match = re.search(r"https://login\.tailscale\.com/[^\s]+", logs)
        if match:
            login_url = match.group(0).rstrip(").,")

    return container, ts_container, login_url or "https://login.tailscale.com"
...
async def cmd_create(ctx, ram: str, cpu: int, disk: str, os_type: str, user: discord.Member):
    ...
    try:
        ...
        container, ts_container, login_url = await asyncio.to_thread(
            _provision_vps_sync,
        ...
        db["vps"][vps_id] = {
            ...
            "tailscale_container_id": ts_container.id,
            "owner_id": user.id,
            "ram": ram,
            "cpu": cpu,
            "disk": disk,
        ...
        # DM Embed with Tailscale Login Link
        dm_embed = discord.Embed(
            title="🚀 Your VPS is Ready!",
            description="Authorize this node in your Tailscale dashboard to get your access IP.",
        ...
        dm_embed.add_field(name="🔑 Tailscale Login Link", value=login_url, inline=False)
        dm_embed.add_field(
        ...
        await msg.edit(
            content=f"✅ **VPS Deployed in {elapsed}s!**\n"
                    ...
                    f"📩 Tailscale login link sent to DMs."
        )
...
async def cmd_myvps(ctx):
    ...
    for vid, info in user_vps:
        embed.add_field(
            name=f"ID: {vid}",
            value=(
                f"**OS:** `{info['os']}` | **Specs:** `{info['ram']} RAM / {info['cpu']} CPU`\n"
