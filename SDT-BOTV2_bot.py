...
# ---------------------------------------------------------
# CONFIGURATION - SAME CONFIG.JSON FORMAT IS SUPPORTED
# ---------------------------------------------------------
...
# The original screenshot supplied 12/39/49 as the invite requirements for
# the first/second/last reward examples. These remain editable by admins.
...
def load_db():
    ...
    # Do not overwrite admin customizations. Only add missing defaults.
...
invite_cache = {}
invite_cache_lock = asyncio.Lock()
...
async def refresh_guild_invites(guild):
    if not guild.me or not guild.me.guild_permissions.manage_guild:
        logging.warning("Cannot monitor invites in %s: Manage Server permission is required.", guild.name)
        return
    try:
        ...
        logging.warning("Invite tracking disabled for %s: missing Manage Server permission.", guild.name)
    except Exception:
        logging.exception("Failed to refresh invites for guild %s", guild.id)
...
# ---------------------------------------------------------
def _remove_container_pair(container_name):
    if docker_client is None:
        return
    for name in (container_name, f"ts-{container_name}"):
        try:
...
def _provision_vps_sync(
    ...
    """
    Creates the OS container and a Tailscale sidecar in the SAME network namespace.

    Important:
    - memory is a hard Docker cgroup limit;
    - CPU is a hard Docker quota and cpuset visibility is used where possible;
    - disk uses Docker storage quota. If the host storage driver does not support
      storage_opt=size, provisioning is refused instead of silently giving a
      larger disk;
    - Tailscale uses a real /dev/net/tun interface, not userspace networking,
      so its 100.x IPv4 can accept normal TCP/SSH connections.
    """
    if docker_client is None:
    ...
    try:
        ...
        logging.info("Image %s not found locally. Pulling...", image_tag)
    ...
    # Give each container a dedicated cpuset where possible. We allocate from
    # the host CPU count in a deterministic round-robin manner. CPU quota still
    # enforces the requested CPU limit.
    ...
    # Docker storage quota is the only part that can enforce a real root disk
    # size without changing the requested Docker-based architecture.
    storage_opt = {"size": f"{disk_bytes}B"}
    ...
    root_cmd = (
        ...
        "sshd -t; "
        "cat > /usr/local/bin/sdt-neofetch <<'EOF'\n#!/bin/sh\necho 'SDT-BOTV2 VPS'\necho \"RAM: $SDT_ALLOC_RAM\"\necho \"CPU: $SDT_ALLOC_CPU core(s)\"\necho \"Disk quota: $SDT_ALLOC_DISK\"\necho \"OS: $SDT_ALLOC_OS\"\nEOF\n"
        "chmod +x /usr/local/bin/sdt-neofetch; "
        "if [ ! -e /usr/local/bin/neofetch.real ]; then cp /usr/bin/neofetch /usr/local/bin/neofetch.real 2>/dev/null || true; fi; "
        "cat > /usr/local/bin/neofetch <<'EOF'\n#!/bin/sh\n/usr/local/bin/sdt-neofetch\nEOF\n"
    ...
    try:
        ...
        # Real Tailscale interface. Userspace networking is deliberately NOT used,
        # because it cannot provide a normal inbound SSH endpoint.
        ...
        for _ in range(20):
            try:
                result = ts_container.exec_run(["tailscale", "version"])
            ...
            raise RuntimeError(f"Tailscale daemon did not start: {last_error}")

        # Create an authentication URL. No auth key is required; the admin/user
        # can authorize the node from the generated Tailscale URL.
        auth_url = None
        for _ in range(12):
        ...
        if not auth_url:
            raise RuntimeError(
                "Tailscale did not return a login URL. Check container network access and /dev/net/tun."
            )

        # Give tailscale a moment, then fetch its IPv4 if the node was already
        # authorized (the link will otherwise be waiting for authorization).
        tailscale_ipv4 = None
        for _ in range(5):
...
async def on_member_join(member):
    ...
    # "Fake" is intentionally a conservative account-age flag, not a claim
    # that Discord has classified the member as fake.
...
async def vps_health_loop():
    ...
    for vps_id, info in db.get("vps", {}).items():
        try:
            ...
            # A missing container is not silently deleted; status is retained for
            # admin diagnosis.
...
async def cmd_invite(ctx):
    ...
    fake = stats["fake"]
    embed = discord.Embed(
        title=f"📨 {ctx.author.display_name}'s Invite Stats",
...
class InvitePlanView(discord.ui.View):
    ...
    async def choose(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Store a claim so the reward system remains auditable.
            db = load_db()
...
async def cmd_customize_plans(ctx, plan_id: int, name: str, ram: str, cpu: int, disk: str, prize: str):
    """
    Paid:
      $customize-plans <id> <name> <ram> <cpu> <disk> <prize>
    Invite reward:
      $customize-plans <id> <name> <ram> <cpu> <disk> <invites>

    Quote names containing spaces:
      $customize-plans 1 "BASIC PLAN" 2g 1 10g 1
    """
    try:
        safe_int(plan_id, "id")
        safe_int(cpu, "cpu")
...
async def create_vps_internal(owner, ram, cpu, disk, os_type, plan_id=None, invite_reward=False):
    ...
    # Save only after the VPS has actually been started.
    ...
    dm_embed = discord.Embed(
        ...
        description=(
            ...
            "in PuTTY, Termius, Termux, OpenSSH, etc."
        ),
    ...
    dm_embed.add_field(
        name="🌐 Tailscale IPv4",
        value=f"`{ts_ipv4 or 'Authorize the node first, then run $myvps'}`",
        inline=False
    )
    dm_embed.add_field(
        name="🔐 SSH",
    ...
    try:
        ...
        # Keep the already-created VPS; DM failure should not destroy a valid VPS.
...
async def cmd_create(ctx, ram: str, cpu: int, disk: str, os_type: str, user: discord.Member):
    ...
    try:
        ...
        await ctx.send(
            ...
            f"📩 Credentials and Tailscale login were sent by DM."
        )
    except Exception as err:
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.BadArgument):
        await ctx.send("❌ Invalid argument. Check the command syntax.")
        return
...
if __name__ == "__main__":
    ...
    if not token or token == "YOUR_DISCORD_BOT_TOKEN_HERE":
        raise RuntimeError("Set your real Discord bot token in config.json before starting SDT-BOTV2.")
    bot.run(token)
